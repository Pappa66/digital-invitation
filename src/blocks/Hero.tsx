...
export default function Hero({ caption, groom, bride, date, place, bgImage, variant = 'center', position, entrance, blockStyle }: HeroProps) {
    ...
    <BlockShell position={position} entrance={entrance} blockStyle={blockStyle}>
      <section className={`relative flex min-h-[100dvh] w-full flex-col overflow-hidden bg-neutral-900 px-6 py-16 text-white ${v.section}`}>
        ...
        <div className={`relative z-10 flex flex-col ${v.inner}`}>
          ...
          <h1 className="mt-5 font-heading text-4xl leading-tight sm:text-5xl" style={{ fontFamily: 'var(--font-heading)' }}>
            {bride}
          </h1>
          <span className="my-2 text-sm opacity-60">&amp;</span>
          <h1 className="font-heading text-4xl leading-tight sm:text-5xl" style={{ fontFamily: 'var(--font-heading)' }}>
            {groom}
          </h1>
